...
function SkeletonRoutine() {
  return (
    <div className="space-y-2">
      {[...Array(3)].map((_, i) => (
        <div key={i} className="flex items-center justify-between rounded-md bg-[var(--sidebar-bg)]/40 px-2 py-2">
          <div className="flex items-center gap-2">
            <span className="size-4 rounded bg-[var(--sidebar-bg)] animate-pulse" />
            <span className="block h-3.5 w-24 rounded bg-[var(--sidebar-bg)] animate-pulse" />
          </div>
          <span className="h-4 w-10 rounded bg-[var(--sidebar-bg)] animate-pulse" />
        </div>
      ))}
    </div>
  );
    ...
    <>
      <WidgetCard title="Routines" icon={CheckSquare} loading={loading} onExpand={() => setDetailOpen(true)}>
        {loading ? (
          ...
          <div className="text-xs text-[var(--muted)]">Error loading routines</div>
        ) : (
              ...
              return (
                <div key={group} className="space-y-1.5">
                  <button
                    ...
                    className="flex w-full items-center justify-between rounded-md bg-[var(--sidebar-bg)]/50 px-2 py-1.5 text-xs font-medium text-[var(--foreground)] hover:bg-[var(--sidebar-bg)]/80 transition-colors"
                  >
                    <div className="flex items-center gap-2">
                      {isExpanded ? (
                      ...
                      <span className="text-[var(--muted)]/70">({routines.filter((r) => r.completed).length}/{routines.length})</span>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {routines.map((routine) => (
                        <div
                          ...
                          className={cn(
                            ...
                            routine.completed
                              ? "bg-[var(--sidebar-bg)]/30"
                              : "bg-[var(--sidebar-bg)]/60 hover:bg-[var(--sidebar-bg)]/80"
                          )}
                        >
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                routine.completed
                                  ...
                                  : "border-[var(--card-border)] bg-[var(--background)]/50 hover:border-[var(--muted)]"
                              )}
                            >
                              {routine.completed && (
                                <svg
                              ...
                              {toggling.has(routine.id) && (
                                <Loader2 className="absolute size-3 animate-spin text-[var(--muted)]" />
                              )}
                            </button>
                            <span
                              className={cn(
